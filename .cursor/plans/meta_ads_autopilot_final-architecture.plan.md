---
name: Meta Ads Autopilot Rebuild
overview: Kompletna rekonstrukcja aplikacji Meta Ads Autopilot - automatycznego systemu zarzadzania reklamami Meta dla restauracji. Plan obejmuje szczegolowy flow od dodania posta do utworzenia reklamy w Meta Ads.
todos:
  - id: infra
    content: Inicjalizacja monorepo (package.json, pnpm-workspace.yaml, vercel.json)
    status: pending
  - id: database
    content: Schemat bazy danych Supabase (5 tabel + funkcja increment)
    status: pending
    dependencies:
      - infra
  - id: api-services
    content: "Core services: SupabaseService, MetaApiService, LlmService"
    status: pending
    dependencies:
      - database
  - id: api-restaurants
    content: RestaurantsModule (CRUD + retry-campaign)
    status: pending
    dependencies:
      - api-services
  - id: api-adsets
    content: AdSetsModule (categories, getOrCreateAdSet, targeting)
    status: pending
    dependencies:
      - api-services
  - id: api-posts
    content: PostsModule - glowna logika processPost (5 krokow)
    status: pending
    dependencies:
      - api-adsets
  - id: api-webhook
    content: WebhookModule (Ayrshare integration)
    status: pending
    dependencies:
      - api-posts
  - id: api-scheduler
    content: SchedulerModule (cron wygaszania reklam)
    status: pending
    dependencies:
      - api-services
  - id: frontend-setup
    content: React + Vite setup (api.ts, types.ts, styles.css)
    status: pending
    dependencies:
      - infra
  - id: frontend-pages
    content: "5 stron: Dashboard, Restaurants, AdSetConfig, Events, PostsLog"
    status: pending
    dependencies:
      - frontend-setup
---
